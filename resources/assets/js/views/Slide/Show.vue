<template>
    <link :href="`${app_url}css/front.css`" rel="stylesheet" />
...
<script>
    ...
    export default {
        ...
        methods: {
            init() {
                let $slidemonster = $("#slidemonster");
                let id = slidemonster.length > 0 ? $slidemonster.attr("data-id") : 0;
                ...
                this.slideId = id;
                this.apiToken = $slidemonster.attr("data-key");

                axios({
                    method: 'GET',
                    url: `${base_url}slides/${id}`,
                    headers: {
